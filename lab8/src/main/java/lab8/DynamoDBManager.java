...
// The lab8.DynamoDBManager class sets up a DynamoDB table and loads it with pharmaceutical data
public class DynamoDBManager {
  ...
  public static String getPharmaInfo(String drugName) {
    String pharmaInfo =
        dynamoDB.getTable(PHARMA_TABLE_NAME).getItem("DrugName", drugName).get("Usage").toString();
    return pharmaInfo;
  }
  ...
  private static void createPharmaTable() {
    ...
    attributeDefinitions.add(
        new AttributeDefinition().withAttributeName("DrugName").withAttributeType("S"));

    CreateTableRequest request =
        new CreateTableRequest()
            .withTableName(PHARMA_TABLE_NAME)
            .withKeySchema(
                new KeySchemaElement().withAttributeName("DrugName").withKeyType(KeyType.HASH))
